...
Adapted to use available databases
========================================
*/
...
-- a) Save FK for all databases (example for PM_DB1)
...
PRINT '========================================='
PRINT 'TEST 4: Add inconsistent data (optional)'
PRINT '========================================='
PRINT ''
PRINT 'Skipping data insertion (could fail if tables have constraints)'
PRINT ''
...
PRINT ''
PRINT 'Content of FK_RESTORE_JOB summary:'
SELECT 
    COUNT(*) as 'Total',
    SUM(CASE WHEN cr_end IS NOT NULL AND err_msg IS NULL THEN 1 ELSE 0 END) as 'Created',
    SUM(CASE WHEN fk_already_exists = 1 THEN 1 ELSE 0 END) as 'Already Existed',
    SUM(CASE WHEN data_inconsistency = 1 THEN 1 ELSE 0 END) as 'Data Inconsistency',
    SUM(CASE WHEN err_msg IS NOT NULL THEN 1 ELSE 0 END) as 'Errors'
FROM FK_RESTORE_JOB
WHERE package_id = @last_restore_id

PRINT ''
PRINT 'Detailed FK_RESTORE_JOB (first 10 rows):'
SELECT TOP 10
    job_id,
    fk_name,
    detail_table,
