...
-- Procedura usuwająca klucze obce z danej bazy
...
BEGIN
    ...
    BEGIN TRY
        ...
        PRINT 'Rozpoczynam usuwanie kluczy obcych dla bazy ' + @db + ' (store_id=' + CAST(@store_id AS varchar(10)) + ')'
        ...
        BEGIN
            BEGIN TRY
                ...
                BEGIN
                    ...
                    PRINT 'Usunięto klucz: ' + @fk_name + ' z tabeli ' + @details_tab
                END
                
            END TRY
            BEGIN CATCH
                SET @err_msg = ERROR_MESSAGE()
                PRINT 'BŁĄD przy usuwaniu klucza ' + @fk_name + ': ' + @err_msg
            END CATCH
        ...
        PRINT 'Zakończono usuwanie kluczy obcych dla bazy ' + @db
...
-- Test procedury (OSTROŻNIE - usuwa klucze!)
-- EXEC z3_fk_rmv @db = 'PWX_DB'

-- Sprawdzenie co zostało zapisane i usunięte
-- SELECT * FROM FK_STORE ORDER BY start_dstamp DESC
-- SELECT * FROM FK_STORE_DET WHERE store_id = (SELECT TOP 1 store_id FROM FK_STORE ORDER BY start_dstamp DESC)